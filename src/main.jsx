...
function App() {
  ...
  useEffect(() => {
    ...
    const open = event => {
      ...
      else setGame({...activity,grade:activity.grade,color:grades.find(item=>item.id===activity.grade)?.color,extended:activity.grade===1})
    }
  ...
  return <>
    <header className="site-header">
      ...
      <nav className={menu ? 'nav open' : 'nav'} aria-label="Điều hướng chính">
        ...
        <span id="nav-middle" className="nav-disabled" aria-disabled="true" title="THCS đang chờ lên kế hoạch">THCS <LockKeyhole size={13}/></span>
        <span id="nav-high" className="nav-disabled" aria-disabled="true" title="THPT đang chờ lên kế hoạch">THPT <LockKeyhole size={13}/></span>
    ...
    <main id="top">
      ...
      <section id="lo-trinh" className="section k12-section">
        ...
        <div className="level-grid">
          ...
          <article className="level-card middle-level is-locked" aria-disabled="true"><div className="level-visual"><img src="/images/middle-school.webp" alt="Học sinh THCS"/><span>02</span><strong className="level-lock"><LockKeyhole size={16}/> Chưa mở</strong></div><div className="level-copy"><span>CẤP 2 · LỚP 6–9</span><h3>THCS</h3><p>Nội dung học tập chưa được mở.</p><div><b>Đang chờ lên kế hoạch</b></div><button id="preview-middle" type="button" disabled><LockKeyhole size={17}/> THCS chưa mở</button></div></article>
          <article className="level-card high-level is-locked" aria-disabled="true"><div className="level-visual"><img src="/images/high-school.webp" alt="Học sinh THPT"/><span>03</span><strong className="level-lock"><LockKeyhole size={16}/> Chưa mở</strong></div><div className="level-copy"><span>CẤP 3 · LỚP 10–12</span><h3>THPT</h3><p>Nội dung học tập chưa được mở.</p><div><b>Đang chờ lên kế hoạch</b></div><button id="preview-high" type="button" disabled><LockKeyhole size={17}/> THPT chưa mở</button></div></article>
        </div>
      </section>
...
function LessonModal({ lesson, done, onComplete, close, openGame }) {
  ...
  return <div ref={backdropRef} className="modal-backdrop lesson-backdrop" role="presentation" onMouseDown={e => e.target === e.currentTarget && closeLesson()}>
    <article className={`${fullscreen ? 'lesson-modal is-fullscreen' : 'lesson-modal'} ${slide === 0 ? 'is-theory-page' : ''}`} role="dialog" aria-modal="true" aria-labelledby="lesson-title" style={{ '--lesson-color': content.color }}>
