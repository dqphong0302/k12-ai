  ...
  return <div className="modal-backdrop game-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && close()}>
    <section className="game-modal rich-game-modal leaf-lab-modal" role="dialog" aria-modal="true" aria-labelledby="leaf-lab-title" style={{ '--game-color': game.color }}>
      ...
      {!complete ? <>
        ...
        <footer className="game-modal-footer"><span>{best ? `Kỷ lục: ${best}/3 sao` : 'Mô hình dùng dữ liệu tổng hợp nhỏ; kết quả chỉ phục vụ quan sát.'}</span><div className="live-stars"><Star size={17} fill="currentColor"/> {scoreForMistakes(session.data.mistakes, countDecisionPoints(game))}/3 sao</div></footer>
