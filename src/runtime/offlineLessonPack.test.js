...
test('gói offline bài học chứa nội dung, hướng dẫn, dữ liệu, phiếu và trạng thái audio',()=>{
  ...
  assert.match(html,/part-1\.opus<\/td><td class="ready">Đã cache/)
  assert.match(html,/part-2\.opus<\/td><td class="pending">Cần mở và phát/)
...
test('đường dẫn audio đúng cho bài Tiểu học, THCS và THPT',()=>{
  ...
  assert.equal(resources[0][0],'/audio/lessons/grade-1/lesson-01/part-1-theory.opus')
  assert.equal(resources[1][3],'/audio/middle/grade-6/lesson-01/part-4.opus')
  assert.equal(resources[2][3],'/audio/high/grade-10/lesson-01/part-4.opus')
